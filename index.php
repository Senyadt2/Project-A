<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Document</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.2.0/css/all.min.css">
    <link rel="stylesheet" href="css/style.css">
    <link rel="stylesheet" href="css/login.css">
</head>
<body>
    <header>
        <!-- <img class="logo" src="data:image/png;base64, " alt="logo"> -->
        <i class="fa-sharp fa-solid fa-bars menu"></i>
        <i class="fa-sharp fa-solid fa-x menu clicked"></i>
        <div class="menu-box">
            <i class="fa-sharp fa-solid fa-user"><span class="fa-solid m-hide">login</span></i>
            <i class="fa-solid fa-address-book"><span class="fa-solid m-hide">sign up</span></i>
            <i class="fa-solid fa-right-from-bracket"><span class="fa-solid m-hide">logout</span></i>
        </div>
    </header>
    <div class="sidebar">
        <a class="icon" href="#section1"><i class="fa-sharp fa-solid fa-house side-icons"></i><span class="fa-solid hide t-home">Home</span></a>
        <a class="icon" href="#section2"><i class="fa-sharp fa-solid fa-scroll side-icons"></i><span class="fa-solid hide t-assignment">Assignments</span></a>
        <a class="icon" href="#section3"><i class="fa-sharp fa-solid fa-calendar side-icons"></i><span class="fa-solid hide t-calendar">Calendar</span></a>
    </div>
    <div class="form-box form-show">
        <div class="form-buttons">
          <div id="btn"></div>
            <button type="button" class="toggle-btn">Log In</button>
            <button type="button" class="toggle-btn">Register</button>
          </div>

          <form id="login" class="input-group">
            <input type="text" class="input-field" placeholder="Enter username"required>
            <input type="password" class="input-field" placeholder="Enter password"required>
            
            <input type="checkbox" class="checkbox"><span class="form-desc">Remember password</span>
            <button type="submit"class="submit-btn">Log In</button>
          </form>
          <form id="register" class="input-group">
            <input type="text" class="input-field" placeholder="Enter username"required>
            <input type="password" class="input-field" placeholder="Enter password"required>
            <input type="password" class="input-field" placeholder="Enter email"required>
            <input type="checkbox" class="checkbox"><span class="form-desc">I agree to the terms and conditions</span>
            <button type="submit"class="submit-btn">Register</button>
          </form>

        </div>
    <div class="section" id="section1">
        <div class="content">
            <button href="window.location.href='#section2'" class="explore">Your Deadlines<span class="dots"></span></button>
        </div>
    </div>

    <div class="section" id="section2">
        <div class="content">
                <div class="button-box">
                    <button class="wt btn active">Web Tech</button>
                    <button class="dsa btn">DSA</button>
                    <button class="sad btn" >SAD</button>
                    <button class="stats btn">Stats</button>
                    <button class="java btn">JAVA</button>
                </div>
                <div class="ass-box">
                
                <div class="wrapper">
                    <div class="ass-content ass1">
                        <h1 class="ass-head">Days left</h1>
                        <span class="tens">0</span><span class="ones">8</span>
                        <h1 class="ass-head">Web Technology</h1>
                    </div>
                    <div class="ass-content ass2">
                        <h1 class="ass-head">Days left</h1>
                        <span class="tens">0</span><span class="ones">8</span>
                        <h1 class="ass-head">Data Sturucture and Analysis</h1>
                    </div>
                    <div class="ass-content ass3">
                        <h1 class="ass-head">Days left</h1>
                        <span class="tens">0</span><span class="ones">8</span>
                        <h1 class="ass-head">Systen Analysis and Design</h1>
                    </div>
                    <div class="ass-content ass4">
                        <h1 class="ass-head">Days left</h1>
                        <span class="tens">0</span><span class="ones">8</span>
                        <h1 class="ass-head">Probability and Statistics</h1>
                    </div>
                    <div class="ass-content ass5">
                    <h1 class="ass-head">Days left</h1>
                        <span class="tens">0</span><span class="ones">8</span>
                        <h1 class="ass-head">Web Technology</h1>
                    </div>
                </div>
                </div>

                <!-- <?php
                    $xml = file_get_contents('assignment.xml');
                    $xml_parse = simplexml_load_string($xml);
                    $m = date("m");
                    $d = date("d");
                    $y = date("y");
                    echo $todays_date;
                    foreach($xml_parse as $key){
                        echo "<div class = \"ass-content\">";
                        echo "<h1>" . $key->subject ."</h1>";
                        echo "<p>" . $key->date["dd"] ."/". $key->date["mm"]."/" .$key->date["yy"] ."</p>";
                        echo "</div>";
                    }
                    
                    ?> -->
        </div>
    </div>
    <div class="section" id="section3">
        <div class="content">

            <div class="intro">
                <h1>Just terrific!</h1>
                <p>Use the power of images</p>
            </div>
        </div>
    </div>
</body>
<script src="js/script.js"></script>
</html>